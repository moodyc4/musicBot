...
async def play(ctx, *, request=""):
    ...
    if request:
        url = ""
        isShort = re.search(r'/shorts/', request)
        ...
        # If request is a YT shorts URL
        if (isShort):
            url = request.replace('/shorts/', '/watch?v=')
        # If request is a playlist
